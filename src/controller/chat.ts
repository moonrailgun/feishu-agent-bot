    ...
    for (const provider of this.providers) {
      ...
      provider.registerMessageReceiveEvent(app, async (userId, chatId, query) => {
        ...
        if (!contextService.isLogin) {
          ...
          const { messageId } = await provider.sendMessage(chatId, `请点击 [此处](${provider.authorizeUrl}) 授权访问`);

          // 等待用户完成登录（最多等待5分钟）
          // Wait for user to complete login (up to 5 minutes)
          const success = await contextService.waitLogin();
          if (!success) {
            await provider.sendMessage(chatId, '登录超时，请重试');
            return;
          }
         ...
         */
        await agentService.generateResponse(chatId, query, loginMessageId);
      });
