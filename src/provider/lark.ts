      ...
      'im.message.receive_v1': event => {
        (async () => {
          ...
          if (messageType !== 'text' && messageType !== 'post') {
            await larkService.sendTextMessage(chatId, '请发送文本消息');
            return;
          }
          ...
          if (chatType === 'group') {
            // 暂时移除第一个艾特群组信息(一般为机器人自己)，避免干扰
            content.text = content.text.replace(/^@_user_1\ /, '');
          }
